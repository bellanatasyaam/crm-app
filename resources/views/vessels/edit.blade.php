...
<div class="container py-4">
    <h2 class="mb-4"><i class="fas fa-edit"></i> Edit Vessel: {{ $vessel->name ?? $vessel->vessel_name }}</h2>
    ...
    <form action="{{ route('vessels.update', $vessel->id) }}" method="POST">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <select name="company_id" class="form-select form-control @error('company_id') is-invalid @enderror"
                        {{ $vessel->company_id ? 'disabled' : '' }}>
                        <option value="">-- No Company --</option>
                        @foreach($companies as $cust)
                            <option value="{{ $cust->id }}" 
                                {{ (old('company_id', $vessel->company_id) == $cust->id) ? 'selected' : '' }}>
                                {{ $cust->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    @if ($vessel->company_id)
                        <input type="hidden" name="company_id" value="{{ $vessel->company_id }}">
                    @endif
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" 
                        ...
                        value="{{ old('name', $vessel->name ?? $vessel->vessel_name) }}" required>
                    @error('name')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="port_of_call" 
                            class="form-control @error('port_of_call') is-invalid @enderror" 
                            value="{{ old('port_of_call', $vessel->port_of_call) }}">
                        @error('port_of_call')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="vessel_type" 
                            class="form-control @error('vessel_type') is-invalid @enderror" 
                            value="{{ old('vessel_type', $vessel->vessel_type) }}" required>
                        @error('vessel_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="flag" 
                            ...
                            value="{{ old('flag', $vessel->flag ?? 'Indonesia') }}">
                        @error('flag')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" step="0.01" name="net_tonnage" 
                            class="form-control @error('net_tonnage') is-invalid @enderror" 
                            value="{{ old('net_tonnage', $vessel->net_tonnage) }}">
                        @error('net_tonnage')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" name="year_built" 
                            ...
                            value="{{ old('year_built', $vessel->year_built) }}" min="1800" max="{{ date('Y') }}">
                        @error('year_built')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <select name="status" class="form-select form-control @error('status') is-invalid @enderror" required>
                        <option value="active" {{ old('status', $vessel->status) == 'active' ? 'selected' : '' }}>Active (In service)</option>
                        <option value="maintenance" {{ old('status', $vessel->status) == 'maintenance' ? 'selected' : '' }}>Maintenance (Under repair)</option>
                        <option value="retired" {{ old('status', $vessel->status) == 'retired' ? 'selected' : '' }}>Retired (Out of service)</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
        ...
        <button type="submit" class="btn btn-success btn-lg">
            <i class="fas fa-save"></i> Update Vessel
        </button>
